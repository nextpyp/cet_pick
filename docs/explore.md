...
!!! warning
    ...
    Make sure the 2D tilt-series are *aligned* tilt-series. They also need to have the same `x-y` dimensions as the 3D tomograms. Typically, we recommend using down-sampled tilt-series and 3D tomograms (with x-y dimensions smaller than 2000 pixels).
...
```
In this mode, training-related files will be saved to `exp/simsiam2d3d/test_sample`, including a log file with the loss and all used arguments.
...
```
In this mode, training-related files will be saved to `exp/simsiam3d/test_sample`, including a log file with the loss and all used arguments.
...
```
Here, we are using a trained model from the 300th epoch.
...
Inference output is saved into a .npz file that contains embeddings, corresponding coordinates, original cropped patches from tomogram, names of corresponding tomogram. Output is saved to `exp/simsiam2d3d/test_sample/all_output_info.npz` for `2d3d` mode and `exp/simsiam3d/test_sample/all_output_info.npz`for `3d` mode.
...
To generate the 2D visualization plot, run:
```
...
To generate the 3D tomogram visualization plot, run:
```
...
```
Outputs include two numpy arrays: `*rec3d.npy` contained the 3D tomogram and `*hm3d_simsiam.npy` the color heatmaps. To generate an example visualization plot, the two arrays can be:
...
Interactive sessions require loading of local images (which are already generated by `plot_2d.py`). Local images are generated by `plot_2d.py` under the directory `exp/simsiam2d3d/test_sample/imgs/`. To connect images to the localhost and keep it running in the background, initiate a new screen session from the terminal using `screen`, go to `exp/simsiam2d3d/test_sample/` and run `python -m http.server 7000`. The images will be hosted on `7000`. Detach the screen session.
...
```
On the terminal, it should show the headers of the parquet file, including the local host address for interactive session. For example, here the local host address is `http://localhost:33203/`
