...
class TomoPretrainDetector(BaseDetector):
    def __init__(self, opt):
        super(TomoPretrainDetector, self).__init__(opt)

    def process(self, images, return_time=False):
        with torch.no_grad():
            output = self.model(images)[-1]
            # hm = output['hm'].sigmoid_()
            feature_vec = output['pretext']
            fm = output['fm']
            # print('fm', fm.shape)
            # wh = output['wh']
        ...
        if return_time:
            # return output, , hm, forward_time
            return output, feature_vec, fm, forward_time
        else:
            return output, feature_vec, fm

    def post_process(self, feature_vec, fm, meta, scale=1):
    ...
    def save_detection(self, hm, dets, path, prefix='', name=''):
        ...
        for k, v in dets.items():
            for c in v:
                ...
                if score > 0.17 and z > 25 and z < 90 and x > 20 and x < 500 and y > 20 and y < 500:
