...
from cet_pick.models.loss import FocalLoss, RegL1Loss, RegLoss, UnbiasedConLoss, ConsistencyLoss, PULoss
...
class TomoCRSemiLoss(torch.nn.Module):
    def __init__(self, opt):
        ...
        # self.crit = torch.nn.MSELoss() if opt.mse_loss else PULoss(0.2)
        ...
        self.cr_loss = UnbiasedConLoss(0.02, 0.1)
    ...
    def forward(self, outputs, batch, epoch, phase, output_cr = None):
        ...
        if opt.contrastive and phase == "train":
            ...
            # gt_hm_f = gt_hm_f.squeeze()
            output_hm = output['hm'].reshape(1, -1)
            output_hm = output_hm.squeeze()
            output_cr = output_cr.reshape(1, -1)
