...
def constrained_quaternion(dtype=torch.float32, device=None):
    u = torch.rand(3, 1, dtype=dtype, device=device)
    u[0]=torch.rand(1,1)[0][0]*(-0.1)+0.1
    u[1]=torch.rand(1,1)[0][0]*(-0.1)+0.1
    u[2]=torch.rand(1,1)[0][0]*(-0.2)+1
    # print(u)
    u = F.normalize(u, dim=0)
    
    rad = torch.rand(1,1,dtype=dtype, device=device)[0]*1.5
    quat = torch.stack((
        u[0]*torch.sin(rad),
        u[1]*torch.sin(rad),
        u[2]*torch.sin(rad),
...
def constrained_SO3(dtype=torch.float32, device=None):
    return quaternions_to_SO3(constrained_quaternion(dtype, device))
