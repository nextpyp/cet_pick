...
from cet_pick.models.utils import Shift2d
import logging
...
class NoiseNetwork(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.blindspot:
            ...
            rotated = [torch.rot90(x, k = rot, dims=[2,3]) for rot in (0, 1, 2, 3)]
        ...
        if self.blindspot:
            ...
            # ]
            aligned = [torch.rot90(rotated, rot, dims=[2,3]) for rotated, rot in zip(rotated_batch, (0,3,2,1))]
...
def get_denoise_network(num_layers, heads, head_conv=64):
    model_dn = NoiseNetwork(out_channels=2, blindspot = True)
    model_nn = NoiseNetwork(out_channels=1, blindspot = False, zero_output_weights=True)
    models = nn.ModuleDict()
    models["denoise"] = model_dn
    models["sigma"] = model_nn

    # model['denoise'] = model_dn
    return models
