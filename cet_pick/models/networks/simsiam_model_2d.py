...
class PreUNet(nn.Module):
    ...
    def forward(self, x1, x2):
        # if len(x1.size()) < 4:
        #     x1 = x1.unsqueeze(1) # add channels dim
        if len(x1.shape) > 4:
            x1 = x1.squeeze(dim=1)
        if len(x2.shape) > 4:
            x2 = x2.squeeze(dim=1)
...
class TomoResBase(nn.Module):
    def __init__(self, feature_dim=128, arch=None, bn_splits=16):
        super(TomoResBase, self).__init__()
        ...
        for name, module in net.named_children():
            if name == 'conv1':
                module = nn.Conv2d(1, 64, kernel_size=3, stride=2, padding=3, bias=False)
...
class TomoResClassifier2D(nn.Module):
    def __init__(self, block, layers, heads, head_conv):
        self.inplanes = 64
        self.heads = heads 
        self.deconv_with_bias = False 

        super(TomoResClassifier2D, self).__init__()
        ...
        self.fc = nn.Linear(256*block.expansion, self.out_dim)
        fill_fc_weights(self.fc)
        for head in self.heads:
            classes = self.heads[head]
            if 'proj' in head:
...
class ClusteringModel(nn.Module):
    ...
    def forward(self, x, forward_pass='default'):
        if forward_pass == 'default':
            features = self.backbone.forward_test(x)
...
def get_clustering_net_small(num_layers, heads, head_conv=32, n_clusters=3, nheads=1):
    block_class, layers = resnet_spec[num_layers]
    backbone = TomoResClassifier2D(block_class, layers, heads, head_conv=0)
...
def get_moco2d_net_small(num_layers, heads, head_conv=256):
...
def get_simsiam2d_net_small(num_layers, heads, head_conv = 32, last_k=0):
    block_class, layers = resnet_spec[num_layers]
    # url = '/nfs/bartesaghilab/qh36/3D_picking/cet_pick_github/cet_pick/cet_pick/models/pretrain/resnet8_u64.sav'
