...
class TomoWResNet(nn.Module):
    def __init__(self, arch, block, layers, heads, num_classes=1000, zero_init_residual=True,
                    groups=1, width_per_group=64, replace_stride_with_dilation=None,
                    norm_layer=None, split_factor=1, output_stride=8, dropout_p=None):
        super(TomoWResNet, self).__init__()
        ...
        inplanes_now = inplanes_origin 
        self.layer4 = None
        self.feature_3d = nn.Sequential(nn.Conv3d(inplanes_now, inplanes_now, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm3d(inplanes_now, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True)
            )

        self.avgpool = nn.AdaptiveAvgPool3d((1,1,1))
        self.fc = nn.Linear(inplanes_now, 128)
    ...
    def _load_pretrained(self, checkpoint, inchans=1):
        ...
        if inchans == 1:
            conv1_weights = state_dict['layer0.0.weight']
            print('conv1_weights', conv1_weights.shape)
            state_dict['layer0.0.weight'] = conv1_weights.sum(dim=1, keepdim=True)
        elif inchans != 3:
...
class ResNet(nn.Module):
    ...
    def __init__(self, arch, block, layers, num_classes=1000, zero_init_residual=True,
        ...
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        # inplanes_dict = {1: 16, 2: 12, 4: 8, 8: 6, 16: 4}
        # self.inplanes = inplanes_dict[split_factor]
...
def _tomonet(arch, block, layers, heads, pretrained, progress, **kwargs):
    model = TomoWResNet(arch, block, layers, heads,**kwargs)
    return model 

def wide_resnet28_10_tomo(heads, pretrained=False, progress=True, **kwargs):
    kwargs['width_per_group'] = 64
    return _tomonet('wide_resnet28_10', BasicBlock, [4, 4, 4, 4], heads,
                        pretrained, progress, **kwargs)
def get_simsiam_wresnet(num_layers, heads, head_conv, pretrained=True, **kwargs):
    model = wide_resnet28_10_tomo(heads, pretrained)
    print('model', model)
    if pretrained:
        path = '/nfs/bartesaghilab/qh36/3D_picking/cet_pick_github/cet_pick/cet_pick/models/pretrain/model_best.pth.tar'
...
# # wrn_28_10 = wide_resnet28_10_tomo(heads)
# # print('wrn', wrn_28_10)
# path = '/nfs/bartesaghilab/qh36/3D_picking/cet_pick_github/cet_pick/cet_pick/models/pretrain/model_best.pth.tar'
# model = get_simsiam_wresnet(heads, pretrained=True)
# state_dict_model = torch.load(path)
# checkpoint = state_dict_model['state_dict']
...
#     param = checkpoint[k]
#     print(k, param.shape)
# _load_pretrained(checkpoint,wrn_28_10,inchans=1)
# print('state_dict', state_dict.keys())