    ...
    def __init__(self, block, layers, heads, head_conv):
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                ...
                fc = nn.Conv2d(1, classes, kernel_size = 1, stride = 1, padding=0, bias=True)
                if 'hm' in head:
                    fc[-1].bias.data.fill_(-2.19)
                else:
                    fill_fc_weights(fc)
...
def get_tomo_net(num_layers, heads, head_conv = 32):
    block_class, layers = resnet_spec[num_layers]

    model = TomoResNet(block_class, layers, heads, head_conv=head_conv)
